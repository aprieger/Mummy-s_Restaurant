        ...
        while (done==false) {
            ...
            while (ok==false){
                ...
                if (addPackageId.matches("[-+]?\\d*\\.?\\d+")) {
                    ArrayList<ArrayList> resultsAL = ServiceArea.sendQuery("SELECT COUNT(*) FROM Packages WHERE Package_Id="+addPackageId);
                    //Check if Package Id is in the Packages table
                    if (resultsAL.get(0).get(0).toString().equals("1"))
                       ok=true; 
                    else
